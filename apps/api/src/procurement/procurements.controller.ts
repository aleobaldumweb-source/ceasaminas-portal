...
export class ProcurementsController {
  ...
  @Get() findPublished(@Query('q') q?: string, @Query('status') status?: string) {
    return this.service.findPublished(q?.trim(), status);
  }
  ...
  @Get(':id') async findPublishedById(@Param('id') id: string) {
    const item = await this.service.findPublishedById(id);
    if (!item) throw new BadRequestException('Licitação não encontrada.');
    return item;
  }
  ...
  create(@Body() input: CreateProcurementDto, @CurrentUser() actor: AuthUser) {
  ...
  update(
    @Param('id') id: string,
    @Body() input: UpdateProcurementDto,
