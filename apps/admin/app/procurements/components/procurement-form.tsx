...
type Props = {
  ...
  onUpdate: <K extends keyof ProcurementFormValues>(
    key: K,
  ...
  return (
    <section className="panel procurement-form-panel">
      <div className="section-title">
        <div>
          <p>CADASTRO</p>
          <h2>{editing ? 'Editar licitação' : 'Nova licitação'}</h2>
        </div>
        {editing && (
      ...
      <form onSubmit={onSubmit} className="grid2 procurement-form">
        ...
        <label>
          ...
          <select
            value={form.modality}
            onChange={(event) => onUpdate('modality', event.target.value as ProcurementModality)}
          >
        ...
        <label>
          ...
          <select
            value={form.status}
            onChange={(event) => onUpdate('status', event.target.value as ProcurementStatus)}
          >
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => onUpdate('estimatedValue', event.target.value)}
          />
        </label>
        <label>
          Abertura
          <input
            ...
            onChange={(event) => onUpdate('openingAt', event.target.value)}
          />
        </label>
        <label>
          Prazo
          <input
            ...
            onChange={(event) => onUpdate('deadlineAt', event.target.value)}
          />
        </label>
        <label>
          Setor responsável
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => onUpdate('publishedAt', event.target.value)}
          />
        </label>
        <PublicationWarning warning={warning} />
        <div className="form-actions">
          <button className="primary" disabled={saving}>
            {saving ? 'Salvando...' : editing ? 'Salvar alterações' : 'Criar licitação'}
          </button>
        </div>
      </form>
    </section>
