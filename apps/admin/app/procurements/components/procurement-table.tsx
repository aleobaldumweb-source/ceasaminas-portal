            ...
            return (
              <tr key={item.id}>
                <td>
                  ...
                  <small>{modalityLabels[item.modality]}</small>
                </td>
                <td>
                  <span className={`proc-status ${item.status.toLowerCase()}`}>
                    {statusLabels[item.status]}
                  </span>
                </td>
                <td>
                  <span className={`publication-state ${publication.className}`}>
                    {publication.label}
                  </span>
                      ...
                      <span key={document.id}>
                        ...
                        {props.canEdit && (
                          <button
                            ...
                            onClick={() => void props.onRemoveDocument(item.id, document.id)}
                          >
                            ×
                          </button>
                        )}
                ...
                <td>
                    ...
                    {props.canEdit && (
                      <>
                        ...
                        <button
                          type="button"
                          className="secondary"
                          onClick={() => props.onSelectDocumentTarget(item.id)}
                        >
                          Anexar
                        </button>
                      </>
                  ...
                  {props.documentFor === item.id && (
                    <DocumentUploader
                      title={props.documentTitle}
                      onTitleChange={props.onDocumentTitleChange}
                      onFileChange={props.onDocumentFileChange}
                      onSubmit={props.onUploadDocument}
                    />
                  )}
                </td>
              </tr>
          ...
          {props.items.length === 0 && (
            <tr>
              <td colSpan={7} className="empty">
                Nenhuma licitação corresponde aos filtros informados.
              </td>
            </tr>
          )}
        </tbody>
